...
def region_of_interest(image,width,height1,height2,roiw,roih):
    ...
    src=np.array([[width/2-490,height1],[width/2+560,height1],[width/2+635,height2],[width/2-590,height2]],np.float32)
...
def start():
    ...
    out = cv2.VideoWriter('/home/nvidia/Videos/9-21/original_{}-{}-{} {}-{}.avi'.format(now.year, now.month, now.day, now.hour, now.minute), cv2.VideoWriter_fourcc(*'MJPG'), 30, (1280,720))

    # sliding_window out_img video writer
    out2 = cv2.VideoWriter('/home/nvidia/Videos/9-21/sliding_{}-{}-{} {}-{}.avi'.format(now.year, now.month, now.day, now.hour, now.minute), cv2.VideoWriter_fourcc(*'MJPG'), 30, (1280,720))
   ...
   # crosswalk_counter = Crosswalk_Counter()
	...
	print('cw_sum_old', cw_sum_old)
        ...
        warp = region_of_interest(edge_img, 1280, 545, 605, 1280, 720) #390 420
		...
		cv2.circle(out_img, (x_location, 360), 5, (255,0,255),-1)
            ...
            cv2.circle(out_img, (x_location, 360), 5, (0,0,255),-1)
            x_old = int(x_location)
	

	pid = round(pidcal.pid_control(int(x_location)), 6)
	curve_detector.pid_list.append(pid)
